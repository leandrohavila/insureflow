...
import { createValidationError } from "../utils/field.util"
import { isEmptyAnswer } from "../utils/answer.util"
import {
  isValidCep,
  isValidChassi,
  isValidCnpj,
...
function err(
  ...
  return createValidationError(field, code, message, rule)
}

function ruleMessage(rule: ValidationRuleV1, fallback: string): string {
  return "message" in rule && typeof rule.message === "string"
    ? rule.message
    : fallback
}

function ruleNumberValue(rule: ValidationRuleV1): number | null {
  return "value" in rule && typeof rule.value === "number" ? rule.value : null
}

function rulePatternValue(rule: ValidationRuleV1): string | null {
...
function assertString(
  ...
  if (typeof value !== "string") return [err(field, "invalid_type", message)]
  return []
}

function skipSemanticValidation(context: ValidationContext): boolean {
  return context.profile === "v1" && context.surface === "server"
}

function createNativeValidator(
  id: string,
  kinds: readonly import("../types/index").SemanticFieldKind[],
  validateValue: (
    value: unknown,
    field: FormFieldDescriptor,
...
export const nativeValidators: FieldValidator[] = [
  createNativeValidator("native.short_text", ["short_text"], (value, field) =>
    assertString(value, field),
  ),
  createNativeValidator("native.long_text", ["long_text"], (value, field) =>
    assertString(value, field),
  ),
  createNativeValidator("native.cpf", ["cpf"], (value, field, context) => {
    ...
    if (skipSemanticValidation(context)) return []
  ...
  createNativeValidator("native.cnpj", ["cnpj"], (value, field, context) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    if (skipSemanticValidation(context)) return []
    return isValidCnpj(String(value))
      ? []
  ...
  createNativeValidator("native.cep", ["cep"], (value, field, context) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    if (skipSemanticValidation(context)) return []
    return isValidCep(String(value))
      ? []
  ...
  createNativeValidator("native.date", ["date"], (value, field, context) => {
    ...
    if (context.surface === "client") {
      return isValidDateBr(String(value))
        ? []
        : [err(field, "invalid_date", "Informe uma data válida")]
    }

    return isValidIsoDate(String(value))
      ? []
      : [err(field, "invalid_date", "Informe uma data válida")]
  }),
  createNativeValidator("native.time", ["time"], (value, field) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    return isValidTime(String(value))
      ? []
  ...
  createNativeValidator("native.datetime", ["datetime"], (value, field) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    return isValidDateTime(String(value))
      ? []
  ...
  createNativeValidator("native.plate", ["plate"], (value, field, context) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    if (skipSemanticValidation(context)) return []
    return isValidPlate(String(value))
      ? []
  ...
  createNativeValidator("native.renavam", ["renavam"], (value, field, context) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    if (skipSemanticValidation(context)) return []
    return isValidRenavam(String(value))
      ? []
  ...
  createNativeValidator("native.chassi", ["chassi"], (value, field, context) => {
    ...
    if (stringErrors.length > 0) return stringErrors
    if (skipSemanticValidation(context)) return []
    return isValidChassi(String(value))
      ? []
      : [err(field, "invalid_chassi", "Informe um chassi válido")]
  }),
  createNativeValidator("native.file", ["file"], (value, field) =>
    assertString(value, field, "Informe um arquivo válido"),
  ),
  ...
  createNativeValidator("native.currency", ["currency"], (value, field) => {
    if (!isValidNumber(value) && parseFiniteNumber(value) === null) {
      return [err(field, "invalid_number", "Informe um número válido")]
    }
    return []
  }),
...
export const genericRuleValidators: GenericRuleValidator[] = [
  {
    ruleType: "required",
    validate(value, field, _rule, context) {
      if (!isEmptyAnswer(field, value)) return null
      if (context.mode === "draft" && !context.enforceRequired) return null
    ...
    validate(value, field, rule) {
      ...
      if (value.length < min) {
        return err(
          field,
          "min_length",
          ruleMessage(rule, `Mínimo de ${min} caracteres`),
          "minLength",
        )
      }
      return null
    ...
    validate(value, field, rule) {
      ...
      if (value.length > max) {
        return err(
          field,
          "max_length",
          ruleMessage(rule, `Máximo de ${max} caracteres`),
          "maxLength",
        )
      }
      return null
    ...
    validate(value, field, rule) {
      ...
      if (parsed < min) {
        return err(
          field,
          "min_value",
          ruleMessage(rule, `Valor mínimo: ${min}`),
          "min",
        )
      }
      return null
  ...
  },
  {
    ...
    validate(value, field, rule) {
      const parsed = parseFiniteNumber(value)
      const max = ruleNumberValue(rule)
      if (parsed === null || max === null) return null
      if (parsed > max) {
        return err(
          field,
          "max_value",
          ruleMessage(rule, `Valor máximo: ${max}`),
          "max",
        )
      }
      return null
    ...
    validate(value, field, rule) {
      ...
      if (!regex.test(value)) {
        return err(
          field,
          "pattern",
          ruleMessage(rule, "Formato inválido"),
          "pattern",
        )
      }
      return null
  ...
  },
  {
    ruleType: "oneOf",
    validate(value, field, rule) {
      if (!("values" in rule)) return null
      if (!rule.values.includes(value)) {
        return err(
          field,
          "one_of",
          ruleMessage(rule, "Valor não permitido"),
          "oneOf",
        )
      }
      return null
  ...
  },
  {
    ruleType: "cpf",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidCpf(value)) return null
  ...
  },
  {
    ruleType: "cnpj",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidCnpj(value)) return null
  ...
  },
  {
    ruleType: "cep",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidCep(value)) return null
  ...
  },
  {
    ruleType: "email",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidEmail(value)) return null
  ...
  },
  {
    ruleType: "plate",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidPlate(value)) return null
  ...
  },
  {
    ruleType: "renavam",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidRenavam(value)) return null
  ...
  },
  {
    ruleType: "chassi",
    validate(value, field, rule) {
      if (typeof value !== "string" || isValidChassi(value)) return null
    ...
    validate(value, field, rule) {
      ...
      if (value.length < min) {
        return err(
          field,
          "min_items",
          ruleMessage(rule, `Selecione ao menos ${min} opções`),
          "minItems",
        )
      }
      return null
    ...
    validate(value, field, rule) {
      ...
      if (value.length > max) {
        return err(
          field,
          "max_items",
          ruleMessage(rule, `Selecione no máximo ${max} opções`),
          "maxItems",
        )
      }
      return null
...
export function applyGenericRules(
  value: unknown,
  field: FormFieldDescriptor,
  context: ValidationContext,
): ValidationError[] {
  if (context.profile !== "v2") return []
  const schema = field.validation
  if (!schema || schema.version !== 1 || !schema.rules?.length) return []

  const errors: ValidationError[] = []
  for (const rule of schema.rules) {
    const validator = genericRuleValidators.find((item) => item.ruleType === rule.type)
    if (!validator) continue
    const error = validator.validate(value, field, rule as ValidationRuleV1, context)
