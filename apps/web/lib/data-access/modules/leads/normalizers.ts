...
export function normalizeLead(lead: BackendLead): Lead {
  ...
  return {
    ...
    owner: lead.owner
      ? {
          id: lead.owner.id,
          name: lead.owner.name?.trim() || "Usuário",
          initials: lead.owner.initials?.trim() || "IF",
        }
      : null,
    ...
    nextReactivationAt: lead.nextReactivationAt ?? null,
    lastReactivatedAt: lead.lastReactivatedAt ?? null,
    dealId: lead.dealId ?? null,
    opportunityType: lead.opportunityType ?? null,
    currentInsurer: lead.currentInsurer ?? null,
    currentPolicyNumber: lead.currentPolicyNumber ?? null,
    policyExpiresAt: lead.policyExpiresAt ?? null,
