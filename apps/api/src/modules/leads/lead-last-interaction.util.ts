...
export function serializeLeadRecord(
  ...
  return {
    ...
    dealId: lead.dealId,
    opportunityType:
      (lead as { opportunityType?: string | null }).opportunityType ?? null,
    currentInsurer:
      (lead as { currentInsurer?: string | null }).currentInsurer ?? null,
    currentPolicyNumber:
      (lead as { currentPolicyNumber?: string | null }).currentPolicyNumber ??
      null,
    policyExpiresAt:
      (
        lead as { policyExpiresAt?: Date | null }
      ).policyExpiresAt?.toISOString() ?? null,
